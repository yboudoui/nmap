...
bool create_xmas_packet(struct s_req req, struct s_raw_packet *raw_packet)
{
    ...
    struct tcphdr tcph = build_tcp_header(req.dst.port, req.src.ip, req.dst.ip, 
                                         FIN_FLAG | PSH_FLAG | URG_FLAG);
